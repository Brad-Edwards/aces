...
def _execution_state(participant_address: str) -> ParticipantAutonomousExecutionStateModel:
...
def _batch(participant_runtime: object) -> SimpleNamespace:
    policy = SimpleNamespace(address=_POLICY_ADDRESS, max_in_flight=2)
    states = [_execution_state(address) for address in _PARTICIPANTS]
