"""Single seam for resolving the published ACES contract corpus.
...
* an **installed distribution** finds the corpus shipped as package data under
  ``aces_contracts/_corpus`` (see the wheel/sdist ``force-include`` in
...
_CORPUS_PACKAGE = "aces_contracts"
...
def _bundled_corpus_root() -> Path | None:
    ...
    corpus at ``aces_contracts/_corpus``. ``importlib.resources.files`` returns
...
def corpus_root() -> Path:
    ...
    if source is not None:
        return source
    raise RuntimeError(
        "ACES contract corpus is unavailable: the installed distribution does "
