"""Operator-controlled OCI image trust policy."""
...
# that never actually pins content is rejected rather than trusted.
...
# exact placeholder shape so a default substitution cannot be triggered by a
# plan-author ref that merely starts with the prefix.
_PLACEHOLDER_REF = re.compile(rf"raes-reference/{_REF_PATH_COMPONENT}")


@dataclass(frozen=True)
class ImageTrustPolicy:
    """Decide which container images the OCI driver may realize."""
