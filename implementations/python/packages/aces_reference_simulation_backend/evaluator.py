"""Reference simulation evaluator: portable evaluation result/history envelopes."""
...
class ReferenceSimulationEvaluator:
    """In-process evaluator over simulated runtime snapshots."""
    ...
    def start(self, plan: EvaluationPlan, snapshot: RuntimeSnapshot) -> ApplyResult:
        ...
        return ApplyResult(
            success=True,
            snapshot=snapshot.with_entries(entries, evaluation_results=results, evaluation_history=history),
    ...
    def _result_payload(op: EvaluationOp, now: str) -> dict[str, object]:
        ...
        payload: dict[str, object] = {
            ...
            "observed_at": now,
            "updated_at": now,
            "detail": f"simulation result for {op.address}",
            "evidence_refs": [],
        }
