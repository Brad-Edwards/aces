"""Reference simulation participant runtime: RUN-311 lifecycle transitions."""
...
class ReferenceSimulationParticipantRuntime:
    """In-process participant runtime for simulated episodes."""
    ...
    def initialize(self, request: ParticipantEpisodeInitializeRequest, snapshot: RuntimeSnapshot) -> ApplyResult:
    ...
    def reset(self, request: ParticipantEpisodeResetRequest, snapshot: RuntimeSnapshot) -> ApplyResult:
    ...
    def restart(self, request: ParticipantEpisodeRestartRequest, snapshot: RuntimeSnapshot) -> ApplyResult:
        return self._new_episode(
            request,
            snapshot,
    ...
    def terminate(self, request: ParticipantEpisodeTerminateRequest, snapshot: RuntimeSnapshot) -> ApplyResult:
        address = request.participant_address
        current_state = self._live_predecessor(
            snapshot,
            address,
            "cannot terminate participant {address!r}: no live episode",
        )
        ...
        events = [
            ParticipantEpisodeHistoryEvent(
                ...
                sequence_number=current_state.sequence_number,
                terminal_reason=terminal_reason,
                details={"detail": request.detail, "clock": "simulation_tick"},
            ),
        ]
    ...
    def _allocate_episode_id(self, address: str) -> str:
        ...
        return f"{address}-simulation-episode-{next_index}"
