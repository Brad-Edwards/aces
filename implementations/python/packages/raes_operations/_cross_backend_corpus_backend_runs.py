...
Only stable RAES-side facts cross into a descriptor: the authored scenario
identity/digest, the compiled RAES address sets, backend id + capability profile,
...
``external-artifact-summarized``. RAES never imports APTL-private schemas.
"""
...
def build_libvirt_backend_run(artifact: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_locator": {
            ...
            "command": "raes libvirt evidence validate",
        },
...
def _aptl_summary_descriptor(scenario: Mapping[str, Any], address_sets: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "limitations": [
            ...
            "The authored scenario identity is the RAES-side authored digest both backends consume; byte-level "
...
def _aptl_from_export(
    ...
    rather than silently inheriting the RAES-side values.
    """
