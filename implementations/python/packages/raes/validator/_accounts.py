...
class _AccountsMixin:
    def _verify_accounts(self) -> None:
        for name, account in self._s.accounts.items():
            ...
            elif account.node and not self._is_unresolved_var(account.node) and not self._is_vm_node(account.node):
                self._err(f"Account '{name}' node '{account.node}' must be a VM node")
