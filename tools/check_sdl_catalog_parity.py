...
_REFERENCE_EDGE_EXPECTATIONS: dict[str, tuple[str, str, str, str]] = {
    ...
    "historical_baselines.*.deployment_tenant_ref": (
        "deployment_tenants",
        _SEMANTIC,
        "fatal dangling or disagreement with cell, reset owner, or bindings",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.deployment_cell_ref": (
        "deployment_cells",
        _SEMANTIC,
        "fatal dangling or tenant disagreement",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.reset_owner_relationship_ref": (
        "relationships",
        _SEMANTIC,
        "fatal unless an agreeing adr-087 shared-service binding",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.actors.*.authority_ref": (
        "entities,agents,accounts,derived:node_services",
        _SEMANTIC,
        "fatal dangling or authority-kind mismatch",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.objects.*.writer_actor_ref": (
        "derived:historical_actors",
        _SEMANTIC,
        "fatal dangling or conflicting event writer",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.objects.*.content_ref": (
        "content",
        _SEMANTIC,
        "fatal dangling or unsafe inline/secret-bearing corpus content",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.events.*.actor_ref": (
        "derived:historical_actors",
        _SEMANTIC,
        "fatal dangling or single-writer conflict",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.events.*.object_refs[]": (
        "derived:historical_objects",
        _SEMANTIC,
        "fatal dangling or invalid lifecycle state",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.events.*.predecessor_refs[]": (
        "derived:historical_events",
        _SEMANTIC,
        "fatal dangling, non-preceding, or cyclic",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.events.*.cause_refs[]": (
        "derived:historical_events",
        _SEMANTIC,
        "fatal dangling, non-preceding, or cyclic",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.events.*.relationship_refs[]": (
        "relationships",
        _SEMANTIC,
        "fatal unless owned typed historical links with exact endpoints",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.relationship_refs[]": (
        "relationships",
        _SEMANTIC,
        "fatal dangling, wrong typed detail, cross-baseline endpoints, or multiple owners",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.materialization_bindings.*.object_refs[]": (
        "derived:historical_objects",
        _SEMANTIC,
        "fatal dangling, duplicate authority, or interface-kind mismatch",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    ...
    "historical_baselines.*.materialization_bindings.*.deployment_cell_ref": (
        "deployment_cells",
        _SEMANTIC,
        "fatal dangling or baseline disagreement",
        _HISTORICAL_STATE_SEMANTICS,
    ...
    "historical_baselines.*.materialization_bindings.*.ordering_dependencies[]": (
        "derived:historical_materialization_bindings",
        _SEMANTIC,
        "fatal dangling or cyclic",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.materialization_bindings.*.readback_requirement_refs[]": (
        "derived:historical_readback_requirements",
        _SEMANTIC,
        "fatal dangling or incomplete object coverage",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.readback_requirements.*.object_ref": (
        "derived:historical_objects",
        _SEMANTIC,
        "fatal dangling",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.readback_requirements.*.assertion_refs[]": (
        "assertions",
        _SEMANTIC,
        "fatal unless an observed-state invariant/postcondition over the exact object",
        _HISTORICAL_STATE_SEMANTICS,
    ),
    "historical_baselines.*.readback_requirements.*.observation_boundary_ref": (
