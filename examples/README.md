...
| [`scenarios/paper-agent-loop.sdl.yaml`](scenarios/paper-agent-loop.sdl.yaml) | Paper reference scenario for authored participant action/observation handoff | Disk-backed example test; focused processor compile check for participant behaviors, action contracts, and observation boundaries | Does not prove a concrete coding-agent runner, APTL realization, or broad benchmark capability |
