...
| [`scenarios/authored-historical-state.sdl.yaml`](scenarios/authored-historical-state.sdl.yaml) | Versioned authored history and native materialization contract | Disk-backed parser test plus focused event, ownership, readback, address, and compiler tests | Does not include a product adapter or prove native materialization |
| [`scenarios/deterministic-live-activity.sdl.yaml`](scenarios/deterministic-live-activity.sdl.yaml) | Finite baseline-bound live activity with exact budgets and deterministic identity | Disk-backed parser test plus focused composition, identity, capability, lifecycle, and proof-boundary tests | Does not schedule work, execute a provider client, or establish participant proof |
