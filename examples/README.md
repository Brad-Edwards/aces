...
| [`scenarios/cross-backend-minimal.sdl.yaml`](scenarios/cross-backend-minimal.sdl.yaml) | Smallest scenario resolving inside more than one provisioning backend's realization envelope | Admission and realization on the reference and libvirt backends from one authored scenario, with per-resource correspondence asserted | Both backends run daemon-free drivers, so this shows specification portability across two declared envelopes and claims nothing about live libvirt/QEMU versus container equivalence or fidelity |
