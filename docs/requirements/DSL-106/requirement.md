...
- IMPLEMENTS → CODE_FILE `implementations/python/packages/aces_sdl/_errors.py` (Structured SDL parse and validation error types)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/aces_sdl/parser.py` (Parser surface attaching validator warnings as Scenario advisories)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/aces_sdl/scenario.py` (Scenario model storing non-fatal advisories)
...
- IMPLEMENTS → CODE_FILE `implementations/python/packages/aces_sdl/validator/__init__.py` (Semantic validator aggregating errors and collecting advisories)
