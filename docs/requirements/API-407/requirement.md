...
- DOCUMENTS → GITHUB_ISSUE `Brad-Edwards/aces#201` (Participant Feature Support And Constraint Declaration (API-407))
...
- IMPLEMENTS → CODE_FILE `implementations/python/packages/aces_backend_protocols/capabilities.py` (API-407 participant feature-support capability helper)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/aces_backend_protocols/manifest.py` (API-407 backend manifest feature-support renderer)
