...
- IMPLEMENTS → GITHUB_ISSUE `1067` (Carry resolved realization posture through the backend-facing plan)
- IMPLEMENTS → PULL_REQUEST `1081` (Resolved realization authority plan handoff and enforcement)
- IMPLEMENTS → DOCUMENTATION `docs/decisions/issue-1067-resolved-realization-posture-handoff-preflight.md` (Resolved-authority phase trace, loss-point analysis, and carrier decision)
- IMPLEMENTS → SPEC `contracts/schemas/plans/provisioning-plan-v1.json` (Published provisioning-plan resolved realization authority contract)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_contracts/planning.py` (Typed plan-level resolved realization authority and total lookup)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_contracts/contracts/realization_plans.py` (Portable resolved realization authority contract model and invariants)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_contracts/plan_projection.py` (Lossless resolved-authority plan projection)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_contracts/realization_authority.py` (Portable pre-mutation realization selection validation)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/compiler/realization_authority_posture.py` (Compiler construction of complete resolved author posture)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/planner/realization_authority.py` (Planner completeness, capability, envelope, and runtime evidence enforcement)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/planner/realization_authority_materialization.py` (Planner materialization of delegated and apparatus-aware authority)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/semantics/realization_apparatus_defaults.py` (Canonical apparatus-default resolution)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/semantics/realization_authority_contract.py` (Registered-concern authority contract projection)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_runtime/backend_realization_authority.py` (Backend admission against the planned resolved boundary)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_runtime/control_plane_api_models.py` (Authenticated control-plane resolved-authority decoding)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_runtime/control_plane_submission.py` (Control-plane submission validation of resolved authority)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_runtime/control_plane_execution.py` (Execution handoff preserving planner-produced authority)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_backend_libvirt/realization/_plan.py` (Libvirt pre-mutation enforcement of planned author permission)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_reference_backend/realization.py` (Reference backend pre-mutation enforcement of planned author permission)
- TESTS → TEST `implementations/python/tests/test_issue_1067_resolved_realization_authority.py` (Resolved-authority compilation, planning, contract, completeness, delegation, constraint, and evidence conformance)
- TESTS → TEST `implementations/python/tests/test_libvirt_backend_provisioner.py` (Libvirt rejection and positive constrained-admission controls before driver mutation)
- TESTS → TEST `implementations/python/tests/test_reference_backend_provisioner.py` (Reference adapter realization-authority enforcement and portable snapshot controls)
- TESTS → TEST `implementations/python/tests/test_runtime_control_plane.py` (Runtime control-plane resolved-authority carriage and admission tests)
- TESTS → TEST `implementations/python/tests/test_runtime_control_plane_api.py` (Authenticated HTTP relay integrity and authority-rewrite rejection tests)
