...
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_contracts/realization_authority.py` (Resolved realization-authority selection enforcement)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/compiler/realization_concern_binding.py` (Registered realization-concern resource-address binding)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/compiler/realization_concern_explicitness.py` (Concern-owned authored-leaf explicitness reduction)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/semantics/realization_runtime_common.py` (Concern-specific observation-support posture)
...
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/semantics/realization_support.py` (Exact, constrained, and open manifest admission with observation diagnostics)
- IMPLEMENTS → CODE_FILE `implementations/python/packages/raes_processor/semantics/realization_typed_runtime_projection.py` (Closed typed projection and value-safe commitments)
