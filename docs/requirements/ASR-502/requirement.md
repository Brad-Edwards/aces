...
- DOCUMENTS → GITHUB_ISSUE `OpenRAE/rae#6` (ASR-502: Backend Conformance Suite And Fixture Corpus)
- IMPLEMENTS → GITHUB_ISSUE `OpenRAE/rae#66` (Backend conformance suite and fixture corpus on the current contracts tree)
- IMPLEMENTS → GITHUB_ISSUE `OpenRAE/rae#502` (Backend conformance end-to-end proof with seeded violations)
- IMPLEMENTS → PULL_REQUEST `OpenRAE/rae#136` (Make published backend profiles the conformance authority)
- IMPLEMENTS → PULL_REQUEST `OpenRAE/rae#553` (Add backend conformance proof tests with seeded violations)
- IMPLEMENTS → PULL_REQUEST `OpenRAE/rae#543` (Ship the contract corpus as package data and add a versioned release line)
- IMPLEMENTS → PULL_REQUEST `OpenRAE/rae#642` (Prove real snapshot mutation in provisioning-only backend conformance)
