...
| EC-T02 | Task records must bind scenario material to an evaluation protocol, metrics, unit of analysis, intended use, population/construct, leakage controls or risk disclosures, apparatus constraints or disclosure notes, validity notes, and supporting artifacts. | REFORMS, DOME, leakage, OpenML, Model Cards/Datasheets. | v1 MUST for required disclosure surfaces; v1 SHOULD for context-dependent leakage/validity population completeness. | `ExperimentTaskModel`, `ExperimentEvaluationProtocolModel`, metric key invariant, split/leakage model, apparatus constraints, validity-note minimum, and supporting-artifact minimum. Completeness of domain-specific leakage and validity analysis remains review responsibility. |
| EC-T03 | Scenario/snapshot identity must bind task and run so a run cannot silently execute a different scenario. | OpenML task/run separation; PROV entity/activity lineage; SDL module-composition lineage. | v1 MUST | `validate_experiment_run_against_task` checks task id/version and scenario snapshot identity/digest when task binds a snapshot. Generic scenario refs are id-only so version/digest/path qualifiers cannot be accepted and ignored. For composed SDL, the scenario-snapshot digest identifies the expanded canonical scenario after import resolution and namespace rewriting; fragment path/module/digest evidence is preserved as audit metadata. |
| EC-T04 | Apparatus is scientific instrument context, including processor/backend identity, participant implementation identity/provenance, manifests, compatibility, parameters, stochastic controls, clocks, measurement channels, observed setup evidence, and limitations. | DETER, cyber-emulation V&V, simulation V&V, cross-testbed reproduction; participant-semantics lineage. | v1 MUST | `ExperimentApparatusContextModel` requires canonical processor/backend, selected manifests, compatibility declarations, parameters, stochastic controls, clocks, measurement channels, observed setup evidence, and known limitations. Participant implementation components must identify participant implementation manifests, and experiment runs with participant implementation apparatus must carry participant implementation provenance. Compatibility and measurement-channel refs are id/version declarations only in v1 and reject digest/path qualifiers that no validator binds to concrete payloads. |
| EC-T05 | Processor/backend manifest references must bind to concrete manifest payload identity, digest evidence, and compatibility declarations. | Provenance, FAIR, apparatus-manifest lineage. | v1 MUST | Manifest refs reject path qualifiers and limit digest qualifiers to processor/backend manifest refs with supported schema versions and matching subject ids. Apparatus model validates selected manifest consistency, unique subject/schema manifest selection, component manifest ids, and digest-qualified selected manifests as canonical component manifests; task constraints require processor/backend manifest ref ids to match subject ids; `validate_experiment_apparatus_context_against_manifests` validates payload identities, optional digests, and mutual processor/backend compatibility. |
...
| EC-T17 | Participant-visible observations must not be treated as hidden world truth. | Gym/Gymnasium, PettingZoo, OpenSpiel, POMDP/POSG lineage. | v1 SHOULD | v1 preserves observation/evidence artifacts and participant implementation manifest/provenance references; full participant epistemic state belongs to participant-semantics/runtime contracts. |
...
| EC-T20 | Runtime APIs, persistence, schedulers, and statistical engines must remain out of this design issue. | Pre-flight guardrails, ADR boundary. | future/non-goal | ADR-055 and formal spec keep this issue to contracts/specification/fixtures/tests. |
...
The final bounded pass tightened EC-T02 and EC-T13 without changing scope:
task leakage, apparatus, validity, and supporting-artifact disclosure surfaces
are schema-required, condition assignments with no auditable run-level criteria
are rejected by both JSON Schema and the Python model, and run-allocation
internal coherence is exposed as a named ACES semantic invariant. It also
tightened reference satisfaction so digest/path-qualified requirements are not
silently accepted by identity-only processor, backend, participant, or
apparatus-context references; digest/path evidence binding is reserved for task
evidence requirements and canonical processor/backend manifest payload
validation.

The closure pass after that broadened the same rule to the remaining id-only
reference paths found during review. Generic scenario references, run task
references, study task/run memberships, run-internal result evidence links, and
artifact `satisfies_refs` now reject unsupported qualifiers or bind them through
concrete artifact validators. Apparatus manifest validation now also rejects
ambiguous selected-manifest subjects, manifest path qualifiers, digest-qualified
selected manifest refs that are not canonical component manifests, manifest refs
that lack a concrete processor/backend payload validator, task-required
processor/backend manifests whose manifest id does not match the subject
identity, and mutually incompatible processor/backend manifest payloads.
Condition-assignment refs now reject digest/path qualifiers across all allowed
ref kinds, including manifests, profiles, capabilities, measurement channels,
tasks, and scenario snapshots.

The final closure repair also closed candidate-side qualifier drift: apparatus
compatibility declarations, component compatibility refs, and measurement
channel refs now reject digest/path qualifiers, including explicit null fields,
so id-only task and study criteria cannot be satisfied by run metadata carrying
unvalidated checksum/path claims. Schema-pruned task, processor/backend identity,
artifact satisfaction, and run-result evidence refs now also reject explicit null
qualifier fields in the Python model, matching the published closed-world JSON
Schema surface.
