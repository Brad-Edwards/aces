...
| SIEM/security-monitoring manager runtime inventory (manager modules, listeners, enrolled agents/groups, detection content sets, bounded settings, API/control-plane posture) | Log-management/security-monitoring runtime inventory, not transport bindings, process/unit state, raw config, raw events, or rule semantics | `Node.runtime.security_monitoring_managers` when observed; manager and child refs may be targeted through qualified runtime refs; see ADR-040 |
